...
const LANG_FLAGS: Record<string, string> = { en: '🇺🇸', es: '🇦🇷', pt: '🇧🇷' };
...
interface ImageEditModalProps {
  ...
  onClose: () => void;
  onSave: (data: { title: I18nFields; description: I18nFields; category: string; featured: boolean }) => Promise<void>;
  onDelete: (id: string) => Promise<void>;
  categories?: Category[];
  imageCountByCategory?: Record<string, number>;
}
...
export default function ImageEditModal({
  ...
  const [title, setTitle] = useState<I18nFields>({ en: '', es: '', pt: '' });
  const [desc, setDesc] = useState<I18nFields>({ en: '', es: '', pt: '' });
  ...
  useEffect(() => {
    if (image) {
      const t = (image.title as I18nFields) || {};
      const d = (image.description as I18nFields) || {};
  ...
  async function handleAutoTranslate() {
    ...
    } catch { /* translation fallback silently */ }
