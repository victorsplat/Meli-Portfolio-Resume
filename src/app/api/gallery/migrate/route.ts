...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const doc = await collection.findOne({ _id: objectId }) as Record<string, unknown> | null;
    if (!doc) {
    ...
    const base64Data = (doc.url as string).split(',')[1];
    if (!base64Data) {
    ...
    const blobUrl = await uploadToBlob(filename, buffer.buffer);

    await collection.updateOne(
      { _id: objectId },
      { $set: { url: blobUrl, migratedAt: new Date() } as Record<string, unknown> }
    );

    return NextResponse.json({
      success: true,
      id: (doc._id as { toString(): string }).toString(),
      oldUrl: (doc.url as string).slice(0, 50) + '...',
      newUrl: blobUrl,
    });
  } catch (error) {
