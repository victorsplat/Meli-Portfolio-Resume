...
export default function GalleryAdmin() {
  ...
  const {
    ...
    uploadFiles: files, addUploadFiles,
  ...
  const categories: Category[] = settings?.categories?.items || [];
  ...
  function compressImage(file: File, maxDim = MAX_DIM, quality = JPEG_QUALITY): Promise<CompressResult> {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          canvas.toBlob((blob) => {
            ...
            fr.onload = () => resolve({ name: file.name, preview: fr.result as string, width, height, originalSize: file.size, compressedSize: blob.size });
            fr.onerror = reject;
  ...
  function processFiles(selected: File[]) {
    ...
    const newFiles: import('@/lib/stores/galleryStore').UploadFile[] = [];
    for (const file of imagesOnly) {
      if (files.some((f) => f.originalName === file.name)) continue;
      const id = `${Date.now()}-${newFiles.length}`;
      const url = URL.createObjectURL(file);
      newFiles.push({ id, originalName: file.name, preview: url, file, compressed: false, _isObjectUrl: true } as unknown as import('@/lib/stores/galleryStore').UploadFile);
    }
  ...
  async function compressPending() {
    ...
    for (let i = 0; i < files.length; i++) {
      const f = files[i];
      if (!f.compressed && (f as unknown as { file: File }).file) {
        try {
          const result = await compressImage((f as unknown as { file: File }).file);
    ...
    return files.filter((f) => !(f as unknown as { file: File }).file || f.compressed);
  }

  async function handleUpload() {
    const formResult = uploadFormSchema.safeParse({ title, description, category, featured });
    if (!formResult.success) {
      alert('Form validation: ' + formResult.error.issues.map((e) => e.message).join(', '));
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-bg-app">
        ...
        <div className="grid md:grid-cols-5 gap-6 mb-12">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="md:col-span-3">
            ...
            >
              ...
              <input ref={fileInputRef} type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif,image/avif" onChange={uploading ? undefined : handleFileSelect} className="hidden" />
            </div>
          </motion.div>
            ...
            <div className="card">
              ...
              >
                {uploading
                  ...
                  : files.length > 0
                    ? `↑ ${t('galleryAdmin.uploadAll', { count: String(files.length) })}`
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin">
                {files.map((f) => (
                  <motion.div key={f.id} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="relative flex-shrink-0 group">
                    ...
                    {(f as unknown as { compressedSize: number }).compressedSize && <p className="text-[8px] text-green-600 dark:text-green-400 text-center">{((f as unknown as { compressedSize: number }).compressedSize / 1024).toFixed(0)}KB</p>}
                  </motion.div>
                ))}
              </div>
            </motion.div>
      ...
      <ImageEditModal
        image={selectedImage as import('@/components/ui/image-edit-modal').GalleryImage | null}
        onClose={() => setSelectedImage(null)}
        onSave={async ({ title, description, category, featured }) => {
          await updateMutation.mutateAsync({ id: (selectedImage as GalleryImage)._id, title, description, category, featured });
