...
export function validateCPF(cpf: unknown): cpf is string {
...
export function validateCEP(cep: unknown): cep is string {
...
export function validateImage(image: unknown): { valid: boolean; error?: string } {
...
export function validateApplication(body: ApplicationBody): ApplicationResult {
  ...
  if (!body.birthDate) {
    ...
    const date = new Date(body.birthDate as string);
  ...
  if (body.experience && !allowedExp.includes(body.experience as string)) errors.experience = 'Invalid experience value';

  const allowedAvailability = ['morning', 'afternoon', 'night'];
  if (body.availability && !allowedAvailability.includes(body.availability as string)) errors.availability = 'Invalid availability value';

  if (body.skills && !Array.isArray(body.skills)) errors.skills = 'Skills must be an array';
  if (body.skills && (body.skills as unknown[]).length > 20) errors.skills = 'Too many skills';
  ...
  return {
    ...
    sanitized: {
      ...
      phone: (body.phone as string || '').replace(/\D/g, '').slice(0, 11),
      cpf: (body.cpf as string || '').replace(/\D/g, '').slice(0, 11),
      cep: (body.cep as string || '').replace(/\D/g, '').slice(0, 8),
      ...
      skills: Array.isArray(body.skills) ? (body.skills as string[]).map(s => sanitize(s)).filter(Boolean).slice(0, 20) : [],
    }
  };
}
